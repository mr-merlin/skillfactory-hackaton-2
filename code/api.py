...
def predict():
    ...
    try:
        ...
        logger.info(f"📊 Результат: {result['conversion_probability']} (уверенность: {result['confidence_level']})")
...
def predict_batch():
    ...
    try:
        ...
        if not request_data or 'sessions' not in request_data:
            return jsonify({
                'error': 'Данные сессий не предоставлены'
            }), 400
        ...
        if not isinstance(sessions, list):
            return jsonify({
                'error': 'sessions должен быть списком'
            }), 400
        
        if len(sessions) > 1000:
            return jsonify({
                'error': 'Максимальное количество сессий: 1000'
            }), 400
        ...
        if successful_predictions:
            avg_probability = sum(r['probability'] for r in successful_predictions) / len(successful_predictions)
            high_confidence_count = sum(1 for r in successful_predictions if r['confidence_level'] == 'высокая')
            ...
            response['statistics'] = {
                ...
                'high_confidence_percentage': round(high_confidence_count / len(successful_predictions) * 100, 2) if successful_predictions else 0
            }
        
        logger.info(f"✅ Пакетное предсказание для {len(sessions)} сессий за {response['execution_time']}с")
...
def get_features():
    ...
    return jsonify({
        ...
        'feature_categories': {
            'temporal': [f for f in model.feature_names if any(x in f for x in ['hour', 'week', 'morning', 'afternoon', 'evening', 'night'])],
            'device': [f for f in model.feature_names if any(x in f for x in ['mobile', 'android', 'ios', 'desktop', 'tablet', 'windows', 'macos'])],
            'geographic': [f for f in model.feature_names if any(x in f for x in ['moscow', 'spb', 'city', 'regional'])],
            'behavioral': [f for f in model.feature_names if any(x in f for x in ['hits', 'pages', 'duration', 'engagement', 'activity'])],
            'traffic': [f for f in model.feature_names if any(x in f for x in ['paid', 'organic', 'referral', 'direct'])]
        }
    })
