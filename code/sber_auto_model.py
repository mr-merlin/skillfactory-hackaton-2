...
class SberAutoModel:
    ...
    def define_target_actions(self, hits):
        ...
        target_keywords = [
            'submit', 'success', 'call', 'contact', 'request', 'callback', 
            'claim', 'chat', 'auth', 'phone', 'sms', 'code', 'confirm',
            'start_chat', 'user_message', 'proactive', 'invitation'
        ]
    ...
    def create_features(self, sessions, hits):
        ...
        # Временные признаки
        df['visit_datetime'] = pd.to_datetime(df['visit_date'].astype(str) + ' ' + df['visit_time'].astype(str))
        ...
        city_stats['city_conversion_rate'] = (city_stats['city_conversions'] / city_stats['city_sessions'] * 100).round(2)
        ...
        df = df.merge(city_stats[['city_conversion_rate', 'city_tier', 'city_sessions', 'city_avg_duration', 'city_avg_hits']], 
                     left_on='geo_city', right_index=True, how='left')
        ...
        df['engagement_score'] = (df['total_hits'] * df['unique_pages'] * df['session_duration']) / 1000
    ...
    def prepare_features(self, df):
        ...
        feature_cols = [
            # Базовые признаки
            'visit_number', 'total_hits', 'unique_pages', 'session_duration', 'unique_events',
            
            # Временные признаки
            'visit_hour', 'visit_weekday', 'is_weekend', 'is_workday',
            'is_morning', 'is_afternoon', 'is_evening', 'is_night',
            
            # Признаки устройств
            'is_mobile', 'is_android', 'is_ios', 'is_desktop', 'is_tablet',
            'is_windows', 'is_macos',
            
            # Источники трафика
            'is_paid', 'is_organic', 'is_referral', 'is_direct',
            
            # Поведенческие метрики
            'avg_time_per_page', 'bounce_rate', 'deep_engagement', 'long_session',
            'very_long_session', 'high_activity', 'very_high_activity',
            'events_per_page', 'engagement_score',
            
            # Признаки повторных посещений
            'is_returning', 'is_frequent',
            
            # Географические признаки
            'is_moscow', 'is_spb', 'is_million_plus', 'is_regional_center',
            'city_conversion_rate', 'city_avg_duration', 'city_avg_hits',
            'city_tier_low', 'city_tier_medium', 'city_tier_high', 'city_tier_very_high'
        ]
        ...
        print(f"🌍 Географических признаков: {len([f for f in feature_cols if 'city' in f or 'moscow' in f or 'spb' in f])}")
        print(f"⏰ Временных признаков: {len([f for f in feature_cols if 'hour' in f or 'week' in f or 'morning' in f or 'afternoon' in f or 'evening' in f or 'night' in f])}")
    ...
    def predict(self, data):
        ...
        confidence_level = "высокая" if probability > 0.7 else "средняя" if probability > 0.3 else "низкая"
...
def train_and_save_model():
    ...
    print(f"📈 Улучшение по сравнению с базовой моделью: {((roc_auc - 0.8859) / 0.8859 * 100):.1f}%")
