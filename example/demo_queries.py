...
def test_query(name, data, expected_behavior):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"🎯 Предсказание: {'Конвертирует' if result['will_convert'] else 'Не конвертирует'}")
...
def main():
    ...
    test_query(
        ...
        high_conversion_data,
        "Пользователь с высоким вовлечением, iOS устройством, в пиковые часы должен показать высокую вероятность конверсии"
    )
    ...
    test_query(
        ...
        low_conversion_data,
        "Пользователь с низким вовлечением, десктопом, в неактивные часы должен показать низкую вероятность конверсии"
    )
    ...
    test_query(
        ...
        medium_conversion_data,
        "Пользователь со средними показателями должен показать среднюю вероятность конверсии"
    )
